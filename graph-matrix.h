...
GraphMatrix createGraph(int dimension) {
    ...
    for (i = 0; i < dimension; i++) {
        for (j = 0; j < dimension; j++) {
            graph.matrix[i][j] = (i == j) ? 1 : 0; // Glavna dijagonala matrice sadrzi vrijednost 1.
        }
    }
    ...
    for (i = 0; i < graph->dimension; i++) {
        for (j = 0; j < graph->dimension; j++) {
            if (i == graph->dimension - 1 || j == graph->dimension - 1) {
                newMatrix[i][j] = (i == j) ? 1 : 0; // Glavna dijagonala matrice sadrzi vrijednost 1.
            } else {
...
int isValidIndex(GraphMatrix graph, int from, int to) {
    return from < 0 || to < 0 || from > graph.dimension - 1 || to > graph.dimension - 1;
}
